...
class RoomsDetailView(View):
    def delete(self, request, room_id):
        ...
        if len(bookings) > 0:
            return JsonResponse(
                {"error": "Нельзя удалить комнату. Комната имеет бронь!"}, status=400
            )
        room.delete()
...
class BookingListView(View):  # Список и создание брони
    ...
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Неверный формат даты, нужен YYYY-MM-DD"}, status=400
            )

        if date_start >= date_end:
            return JsonResponse(
                {"error": "date_start должен быть раньше чем date_end"}, status=400
            )

        overlapp = Booking.objects.filter(
            room=room, date_start=date_start, date_end=date_end
        ).exists()
        if overlapp:
            return JsonResponse(
                {"error": "Номер уже забронирован на эти даты"}, status=400
            )
